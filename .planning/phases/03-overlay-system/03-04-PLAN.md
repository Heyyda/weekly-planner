...
must_haves:
  truths:
    ...
    - "Multi-monitor enumeration использует ПУРНЫЙ ctypes EnumDisplayMonitors (БЕЗ pywin32) — per D-19 (нет optional dependency)"
  ...
  artifacts:
    - path: "client/ui/overlay.py"
      provides: "OverlayManager — создание окна, drag, click, position persistence, ctypes multi-monitor"
  ...
  key_links:
    ...
    - from: "client/ui/overlay.py multi-monitor"
      to: "ctypes.windll.user32.EnumDisplayMonitors"
      via: "MONITORENUMPROC callback (D-19: чистый ctypes, не pywin32)"
      pattern: "windll\\.user32\\.EnumDisplayMonitors"
---
...
Multi-monitor implementation (D-19): **чистый ctypes** через
`ctypes.windll.user32.EnumDisplayMonitors` + MONITORENUMPROC callback + `GetMonitorInfoW`.
НЕ используем `pywin32` (НЕ добавлен в requirements.txt — optional dependency убрана по D-19;
silent fallback к primary monitor при отсутствии pywin32 — деградация UX, недопустимо).
...
<!-- Ctypes types needed for MONITORENUMPROC (D-19) -->
Win32 signatures (pure ctypes, без pywin32):
```python
# user32.EnumDisplayMonitors(hdc, lprcClip, lpfnEnum, dwData) -> BOOL
# MONITORENUMPROC = WINFUNCTYPE(BOOL, HMONITOR, HDC, LPRECT, LPARAM)
# user32.GetMonitorInfoW(hMonitor, lpmi) -> BOOL
...
<task type="auto" tdd="true">
  <name>Task 1: Создать OverlayManager — окно + drag + click + position persistence + pure-ctypes multi-monitor (D-19)</name>
  ...
  <read_first>
    ...
    - .planning/phases/03-overlay-system/03-RESEARCH.md Tertiary refs — screeninfo.enumerators.windows (пример EnumDisplayMonitors через ctypes)
    - .planning/phases/03-overlay-system/03-UI-SPEC.md §Overlay, §Drag Behavior, §Click Behavior, §Always-on-top
    - .planning/phases/03-overlay-system/03-CONTEXT.md D-19 (чистый ctypes для multi-monitor — БЕЗ pywin32)
    - client/ui/settings.py (UISettings.overlay_position + on_top)
    - client/ui/icon_compose.py (render_overlay_image API)
    - requirements.txt — подтвердить что pywin32 отсутствует (D-19 requires чистый ctypes)
  </read_first>
  ...
  <behavior>
    ...
    - **Test 13 (D-19): _get_virtual_desktop_bounds использует `ctypes.windll.user32.EnumDisplayMonitors` (не win32api.EnumDisplayMonitors). Проверка: inspect.getsource НЕ содержит `import win32api` и содержит `windll.user32.EnumDisplayMonitors`**
    - **Test 14 (D-19): _get_virtual_desktop_bounds возвращает валидный tuple (left, top, right, bottom) на реальной системе (через headless_tk — хотя бы primary screen bounds)**
  </behavior>
  ...
  <action>
    ...
    Ref: screeninfo.enumerators.windows (github.com/rr-/screeninfo) — пример MONITORENUMPROC.
    """
    ...
    # Win32 structs and callback signature for EnumDisplayMonitors.
    ...
    class OverlayManager:
        ...
        def _get_virtual_desktop_bounds(self) -> tuple:
            ...
            Fallback на primary screen при любой ошибке ctypes (не Windows / нестандартная среда).
            """
            rects: list = []
    ...
    ШАГ 4 — Убедиться что `requirements.txt` НЕ содержит `pywin32` (D-19: чистый ctypes).
    Если после рефакторинга кто-то случайно добавит pywin32 — удалить.
  </action>
  ...
  <acceptance_criteria>
    ...
    - **grep "windll.user32.EnumDisplayMonitors" client/ui/overlay.py >= 1 (D-19: pure ctypes path)**
    - **grep "MONITORENUMPROC\\|WINFUNCTYPE" client/ui/overlay.py >= 1 (D-19: callback signature)**
    - **grep -c "import win32api\\|from win32api\\|win32api\\." client/ui/overlay.py == 0 (D-19: БЕЗ pywin32)**
    - **grep "^pywin32" requirements.txt == 0 (D-19: pywin32 не в deps, чистый ctypes достаточно)**
    - pytest client/tests/ui/test_overlay.py проходит >= 14 тестов зелёными (включая test_multi_monitor_uses_pure_ctypes_not_pywin32 и test_virtual_desktop_bounds_returns_tuple)
  </acceptance_criteria>

  <done>
    OverlayManager готов. OVR-01, 02, 03, 04, 06 покрыты. D-19 выполнен: multi-monitor через
    чистый ctypes (EnumDisplayMonitors + MONITORENUMPROC + GetMonitorInfoW) — никаких
    optional pywin32-зависимостей. Plan 03-05 будет вызывать overlay.refresh_image(state="overdue",
    pulse_t=t) в 60fps цикле. Plan 03-10 wire'нет on_click → main_window.toggle. Все три
    критические pitfall'ы встроены с grep-verified markers.
  </done>
</task>
...
<verification>
- pytest client/tests/ui/test_overlay.py — все тесты зелёные (14+)
- grep markers для PITFALL 1 (after 100), 4 (_tk_image), 6 (_validate_position) подтверждены
- D-19 verified: grep "windll.user32.EnumDisplayMonitors" присутствует, grep "win32api" ОТСУТСТВУЕТ
- requirements.txt не содержит pywin32 (D-19)
...
- Multi-monitor использует чистый ctypes (D-19), без optional pywin32 dependency
