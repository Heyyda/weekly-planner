...
- [x] **BOT-03**: Команда `/add <текст>` — быстро создать задачу на сегодня
- [x] **BOT-04**: Команда `/today` и `/week` — просмотр текущего дня / текущей недели
- [x] **BOT-05**: Кнопка "отметить выполненной" на inline-клавиатуре под каждой задачей
